...
def kurvendiskussion_polynome(nr, teilaufg):
    ...
    if 'c' in teilaufg:
        ...
        table2.add_row('Berechnung der Partialfunktion  mit Hornerschema: ', '', fkt_a2, fkt_a3, fkt_a4)
        table2.add_hline(2, 5)
        table2.add_row('', '', fkt_b2, fkt_b3, fkt_b4)
        table2.add_hline(2, 5)
        ...
        loesung.append(str(liste_teilaufg[i]) + r') \quad \mathrm{Ansatz:~f(x)~=~0} \quad \to \quad 0~=~' + latex(fkt)
                       + r' \quad \mathrm{durch~probieren:~x_1~=~}' + latex(nst_1)
                       + r' \quad (2P) \\' + '(' + latex(fkt) + r')~ \div ~(x' + vorz_str(-1 * nst_1) + ')~=~'
        ...
        loesung.append(r'\hspace{10em} \\' + latex(fkt_partial) + r'~=~0 \quad \vert ~ \div '
                       ...
                       + gzahl_klammer(fkt_partial_q) + r'} \quad (2P) \\' + r' x_2~=~' + latex(round(nst_2, 3))
                       + r' \quad \mathrm{und} \quad x_3~=~' + latex(round(nst_3, 3)) + r' \quad (2P) \\'
                       + r'S_{x_1}(' + latex(nst_1) + r'\vert 0) \quad S_{x_2}(' + latex(round(nst_2, 3))
                       + r' \vert 0) \quad S_{x_3}(' + latex(round(nst_3, 3)) + r' \vert 0)' + lsg
    ...
    if 'd' in teilaufg:
        ...
        if fkt_2.subs(x, x_1_fkt_1) < 0:
            loesung_f_monotonie_1 = (r'~<~0~ \to HP(~' + latex(x_1_fkt_1) + r'~ \vert ~'
                                     + latex(round(fkt.subs(x, x_1_fkt_1), 3)) + r') \quad (3P) \\')
        else:
            loesung_f_monotonie_1 = (r'~>~0~ \to TP(~' + latex(x_1_fkt_1) + r'~ \vert ~'
                                    + latex(round(fkt.subs(x, x_1_fkt_1), 3)) + r') \quad (3P) \\')

        if fkt_2.subs(x, x_2_fkt_1) < 0:
            loesung_f_monotonie_2 = (r'~<~0~ \to HP(~' + latex(x_2_fkt_1) + r'~ \vert ~'
                                     + latex(round(fkt.subs(x, x_2_fkt_1), 3)) + r') \quad (3P) \\')
        else:
            loesung_f_monotonie_2 = (r'~>~0~ \to TP(~' + latex(x_2_fkt_1) + r'~ \vert ~'
                                     + latex(round(fkt.subs(x, x_2_fkt_1), 3)) + r') \quad (3P) \\')
        ...
        loesung.append(str(liste_teilaufg[i]) + r') \quad f^{ \prime }(x) ~=~' + latex(fkt_1)
                       ...
                       + r'x_1~=~' + latex(x_1_fkt_1) + r' \quad \mathrm{und} \quad x_2~=~' + latex(x_2_fkt_1)
                       + r' \quad (2P) \\' + r' f^{ \prime \prime }(' + latex(x_1_fkt_1) + ')~=~'
                       + latex(round(fkt_2.subs(x, x_1_fkt_1), 3)) + loesung_f_monotonie_1 + r' f^{ \prime \prime }('
                       + latex(x_2_fkt_1) + ')~=~' + latex(round(fkt_2.subs(x, x_2_fkt_1), 3))
    ...
    if 'e' in teilaufg:
        ...
        xwert_Wendepunkt = Rational(2 * faktor * (nst_1 + nst_2 + nst_3), 6 * faktor)
        ...
        loesung.append(str(liste_teilaufg[i]) + r') \quad f^{ \prime \prime }(x) ~=~0 \quad \to \quad 0~=~'
                       + fkt_2_str + r' \quad \vert ~-~' + gzahl_klammer(-2 * faktor * (nst_1 + nst_2 + nst_3))
                       + r' \quad \vert \div ' + gzahl_klammer(6 * faktor) + r' \quad \to \quad x_1~=~'
                       + latex(xwert_Wendepunkt) + r' \quad (2P) \\ f^{ \prime \prime \prime }('
                       + latex(xwert_Wendepunkt) + r') \quad \neq 0 \quad \to \quad WP('
                       + latex(xwert_Wendepunkt) + r' \vert ' + latex(round(fkt.subs(x, xwert_Wendepunkt), 3))

...
def lineare_funktionen(nr, teilaufg=['a', 'b', 'c', 'd', 'e', 'f'], anz_einf=list(range(8))[1], anz_pkt=list(range(8))[1], notizfeld=[False, True][0], neue_seite=[0,1,2,3,4,5,6][0], i=0, BE=[]):
    ...
    aufg_e = 1 if bool(set(teilaufg) & {'e', 'f'}) else 0
    liste_fkt_m = [-2.5,-2,-1.5,-1,-0.5,0.5,1,1.5,2,2.5]
    liste_fkt_n = list(range(-3,4))
    fkt_m = random_selection(liste_fkt_m, anz_einf + aufg_e)
    fkt_n = random_selection(liste_fkt_n, anz_einf + aufg_e)
    del liste_fkt_m
    del liste_fkt_n
    ...
    if bool(set(teilaufg) & {'a', 'b', 'c'}):
        ...
        lsg = (beschriftung(len(teilaufg), i, True) + r'  \mathrm{die~Funktionsgleichung(en):} \hspace{10em} \\')
        ...
        if anz_einf + anz_pkt == 1:
            aufgabe.extend(('Gegeben sind die folgenden Graphen', 'Grafik',
    ...
    if 'b' in teilaufg:
        ...
        lsg = (beschriftung(len(teilaufg), i, True) + r' \mathrm{die~Nullstellen~werden~berechnet~mit~'
               + r'x_0~=~ - \frac{n}{m}} \\')
        for step in range(anz_einf):
    ...
    if 'c' in teilaufg:
        ...
        def tabelle(fkt, fkt_str, bez):
            ...
            table1.add_hline(2,7)
            return table1
        loesung.append(beschriftung(len(teilaufg), i, True) + r'  \mathrm{Wertetabelle~für~die~aufgestellte(n)~'
...
def einf_parabeln(nr, teilaufg=['a', 'b', 'c', 'd', 'e', 'f'], anz_np=[0,1,2,3,4,5,6][1], anz_ap=[0,1,2,3,4,5,6][1],  notizfeld=[False, True][0], neue_seite=[0,1,2,3,4,5,6][0], i=0, BE=[]):
    ...
    if bool(set(teilaufg) & {'a', 'b', 'c'}):
        ...
        # Lösungen für Gleichungen
        lsg = (beschriftung(len(teilaufg), i, True)
               + r'  \mathrm{abgelesener~Scheitelpunkt,~ggf.~der~Faktor~und~die~Funktionsgleichung:} \\')
    ...
    if 'b' in teilaufg:
        ...
        # Lösungen für Gleichungen
        lsg = (beschriftung(len(teilaufg), i, True)
               + r' \mathrm{umgeformte~Funktionsgleichung~lauten:} \hspace{15em}  \\')
        for step in range(anz_np):
        ...
        if anz_np + anz_ap == 1:
            aufgabe.append(NoEscape(r' \noindent ' + beschriftung(len(teilaufg), i) + f'Gib die Funktionsgleichung '
                                    + f'aus Teilaufgabe a) auch in der Normalform an.'))

        else:
            aufgabe.append(NoEscape(r' \noindent ' + beschriftung(len(teilaufg), i) + f'Gib alle Funktionsgleichungen '
                                    + f'aus Teilaufgabe a) auch in der Normalform an.'))
        loesung.append(lsg)
        if notizfeld:
            aufgabe.append(['Bild', '430px'])
    ...
    if 'c' in teilaufg:
        ...
        # Lösungen für Gleichungen
        lsg = (beschriftung(len(teilaufg), i, True)
               + r' \mathrm{Nullstellen~mit~p~q~Formel~berechnen:} \hspace{15em} \\')
        for step in range(anz_np):
            lsg += r' \mathrm{Nullstellen~der~Funktioen~' + fkt_bez[step] + r'(x) ~ : } \hspace{15em} \\'
            lsg_text, lsg_werte, lsg_pkt = quadr_gl([1,-2 * xwert_s[step], xwert_s[step] ** 2 + ywert_s[step]])
            lsg += lsg_text[0]
            punkte += lsg_pkt
            lsg = lsg + r' \\ ' if step + 1 < anz_np + anz_ap else lsg

        for step in range(anz_ap):
            lsg += r' \mathrm{Nullstellen~der~Funktioen~' + fkt_bez[anz_np + step] + r'(x) ~ : } \hspace{15em} \\'
            lsg_text, lsg_werte, lsg_pkt = quadr_gl([fakt_ap[step],-2 * fakt_ap[step] * xwert_s[anz_np + step],
                                                     fakt_ap[step] * xwert_s[anz_np + step] ** 2
                                                     + ywert_s[anz_np + step]])
            lsg += lsg_text[0]
            punkte += lsg_pkt
            lsg = lsg + r' \\ ' if (anz_np + step + 1) < anz_np + anz_ap else lsg
    ...
    if 'd' in teilaufg:
        ...
        fkt_d_spf_str = (r' \left( x' + vorz_str(-1 * xwert_s[k]) + r' \right) ^2 '
                         + vorz_str(ywert_s[k]))
        punkte = 3
        aufgabe.extend((NoEscape(r' \noindent ' + beschriftung(len(teilaufg), i) + 'Zeichne den Graphen von $'
                                 + bez_fkt_d + '$(x) = $' + fkt_d_spf_str + '$ im Koordinatensystem ein.'), ' \n\n'))
        loesung.append(beschriftung(len(teilaufg), i, True) + r' \mathrm{Scheitelpunkt~(1BE) \quad Graph~(1BE) \quad '

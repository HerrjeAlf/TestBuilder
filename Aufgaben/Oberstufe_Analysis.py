...
def folgen(nr, teilaufg=['a', 'b', 'c', 'd'], ausw_folgenart=None, i=0, BE=[]):
    ...
    if 'c' in teilaufg:
        ...
        loesung.append(str(liste_teilaufg[i]) + r') \quad a_n~=~' + a_n_str + r' \quad (2BE)')
        liste_punkte.append(3)
        i += 1
...
def grafisches_ableiten(nr, teilaufg=['a', 'b'], i=0, BE=[]):
    ...
    if len([element for element in ['a', 'b'] if element in teilaufg]) > 0:
        ...
        loesung.extend((str(liste_teilaufg[i]) + r') \quad \mathrm{~Graph~der~Ableitungsfunktion~(2BE)} ', 'Figure'))
...
def kurvendiskussion_exponentialfkt_parameter(nr, teilaufg=['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j'], ableitung=None, i=0, BE=[]):
    ...
    if 'f' in teilaufg:
        ...
        liste_bez.append(f'{str(nr)}.{str(liste_teilaufg[i])})')
        punkte = 4
        aufgabe.append(str(liste_teilaufg[i]) + ') Erläutern Sie die Abhängigkeit der Extremwerte und deren Art '

...
def aenderungsrate(nr, teilaufg):
    ...
    max_interval = 5.5
    runs = 0
    while ((faktor * (x_wert_1 - s_xwert) ** 2 + s_ywert) > max_interval or (
            faktor * (x_wert_1 - s_xwert) ** 2 + s_ywert) < -max_interval) or (
            (faktor * (x_wert_2 - s_xwert) ** 2 + s_ywert) > max_interval or (
            faktor * (x_wert_2 - s_xwert) ** 2 + s_ywert) < -max_interval):
        s_xwert = zzahl(1, 3)
        x_wert_1 = s_xwert + abstand[0]
        x_wert_2 = s_xwert + abstand[1]
    ...
    print(f'Interval [X:{x_wert_1} Y:{faktor * (x_wert_1 - s_xwert) ** 2 + s_ywert} | X:{x_wert_2} Y:{faktor * (x_wert_2 - s_xwert) ** 2 + s_ywert}]')
    ...
    if a in teilaufg:
        aufgabe.append(str(liste_teilaufg[
                               i]) + f') Bestimme zeichnerisch die mittlere Änderungsrate im Interval [ {x_wert_1} | {x_wert_2} ] vom Graphen f. \n\n')
        ...
        if y_wert_1 == y_wert_2:
            ...
            loesung.append(str(liste_teilaufg[
                                   i]) + r') \quad \mathrm{Gerade~durch~beide~Punkte~(1P),~~Steigungsdreieck~(1P),~~m~bestimmt~(1P)} \\')
            Punkte += 3
    ...
    if b in teilaufg:
        aufgabe.append(str(liste_teilaufg[
                               i]) + f') Überprüfe die mittlere Änderungsrate im Interval [ {x_wert_1} | {x_wert_2} ] durch Rechnung. \n\n')
    ...
    if d in teilaufg:
        aufgabe.append(str(liste_teilaufg[
                               i]) + f') Überprüfe die lokale Änderungsrate an der Stelle x = {x_wert_2} mit einer Rechnung. \n\n')
...
def Hausaufgabenkontrolle():
    ...
    with Aufgabe.create(Figure(position='h!')) as graph:
        graph.add_image(r'S:\Schule\Aufgabe_4.png', width='400px')
...
def Erwartungshorizont():
    ...
    with Loesung.create(Figure(position='h!')) as graph:
        graph.add_image(r'S:\Schule\loesung_Aufgabe_4.png', width='400px')
